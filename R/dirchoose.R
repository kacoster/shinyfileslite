    ...
    dirGetter <-
      function(roots, restrictions, filetypes, hidden = FALSE) {
        ...
        function(tree, root) {
          ...
          tree <-
            traverseDirs(tree, currentRoots[root], restrictions, hidden)
    ...
    dirCreator <- function(roots, ...) {
      ...
      function(name, path, root) {
        ...
        if (is.null(names(currentRoots)))
          stop("Roots must be a named vector or a function returning one")
        location <-
          fs::path(currentRoots[root], paste0(c(path, name), collapse = "/"))
        dir_create(location)
      }
    }
    ...
    shinyDirChoose <- function(input,
      ...
      sendDirectoryData <- function(message) {
        ...
        if (is.null(tree) || is.na(tree)) {
          dir <-
            list(tree = list(name = defaultPath, expanded = TRUE),
                 root = defaultRoot)
          files <- list(dir = NA, root = tree$selectedRoot)
        } else {
          dir <- list(tree = tree$tree, root = tree$selectedRoot)
          files <-
            list(dir = unlist(tree$contentPath),
                 root = tree$selectedRoot)
      ...
      function(id,
        ...
        tagList(
          ...
          tags$a(
            ...
            type = "button",
            class = paste(c(
              "shinyDirectories", class, "action-button"
            ), collapse = " "),
            style = style,
            "data-title" = title,
            "data-val" = value,
